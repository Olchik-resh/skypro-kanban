...
async function handleSubmit(event) {
  ...
  if (!isValid) {
    if (props.isSignUp) {
      error.value = 'Введённые вами данные некорректны. Чтобы завершить регистрацию, заполните все поля в форме.';
    } else {
      error.value = 'Введённые вами данные не распознаны. Проверьте свой логин и пароль и повторите попытку входа.';
    }
    return; // Добавим возврат, чтобы предотвратить дальнейшие действия при недействительной форме
  }
