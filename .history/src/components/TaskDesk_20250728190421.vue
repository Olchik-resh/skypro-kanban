...
const noStatusTask = computed(() => {
  return tasks.value.filter((task) => task.status === 'Без статуса');
});
const doStatusTask = computed(() => {
  return allTasks.value.filter((task) => task.status === 'Нужно сделать');
});

const workStatusTask = computed(() => {
  return allTasks.value.filter((task) => task.status === 'В работе');
});

const testStatusTask = computed(() => {
  return allTasks.value.filter((task) => task.status === 'Тестирование');
});

const readyStatusTask = computed(() => {
  return allTasks.value.filter((task) => task.status === 'Готово');
});
</script>
