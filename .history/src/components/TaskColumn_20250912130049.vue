...
<div class="column">
  ...
  <div class="column__cards-wrapper">
    <TaskCard
      ...
      v-bind="task"
    />
    <!-- Если задач нет и loading, показываем скелетоны -->
    <TaskCardSkeleton v-if="tasks.length === 0 && loading" v-for="n in 3" :key="n" />
    <!-- Если задач нет и не loading, пишем "Нет задач" -->
    <div v-if="tasks.length === 0 && !loading">Нет задач</div>
  </div>
  <!-- Оверлей поверх карточек при loading и наличии задач -->
  <div v-if="tasks.length > 0 && loading" class="column__overlay"></div>
</div>
</template>
