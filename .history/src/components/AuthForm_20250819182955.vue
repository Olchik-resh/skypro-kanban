...
function validateForm() {
  ...
  if (!isValid) {
    if (props.isSignUp) {
      error.value = 'Введённые вами данные не корректны. Чтобы завершить регистрацию, заполните все поля в форме.';
    } else {
      error.value = 'Введённые вами данные не распознаны. Проверьте свой логин и пароль и повторите попытку входа.';
    }

    // Устанавливаем флаг недействительности формы и делаем кнопку серой только при отправке формы
    if (isFormSubmitAttempt) { // флаг, который устанавливается при попытке отправки формы
