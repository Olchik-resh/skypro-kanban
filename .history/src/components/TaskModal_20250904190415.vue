...
const handleDateSelect = (date) => {
  ...
  dateError.value = dayjs(date).isBefore(dayjs().startOf('day'))
...
const validateForm = () => {
  ...
  return !titleError.value && !dateError.value && !statusError.value
}
...
const saveChanges = async () => {
  try {
    ...
    console.log('Текущая дата:', dayjs().format())
    console.log('Дата задачи:', dayjs(editedTask.value.date).format())
    ...
    const taskData = {
      ...
      date: dayjs(editedTask.value.date).toISOString(), // Преобразование в ISO формат
    }
...
const handleDateSelect = (date) => {
  ...
  formError.value = '' // Сброс других ошибок
}
watch(
  editedTask,
  () => {
