...
const isFormInvalid = computed(() => {
  ...
  if (!formData.value.login.trim()) {
    return true // Если логин не введён, форма недействительна
  }
  if (!formData.value.password.trim()) {
    return true // Если пароль не введён, форма недействительна
  }
...
async function handleSubmit(event) {
  if (isFormInvalid.value) event.preventDefault()
