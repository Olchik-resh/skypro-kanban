    ...
    <div class="calendar__block">
      ...
      <div class="calendar__period">
        <p class="calendar__p date-end">
          {{ dateLabel }} <span class="date-control">{{ selectedDateFormatted }}</span>
        </p>
      </div>
    </div>
  </div>
...
const selectedDateFormatted = computed(() =>
  selectedDate.value ? selectedDate.value.format('DD.MM.YY') : '—',
)
...
const dateLabel = computed(() =>
  selectedDate.value ? 'Срок исполнения:' : 'Выберите срок исполнения',
)
</script>
