...
function validateName(name) {
  // Проверка на заполненность и ограничение по длине
  const maxLength = 50
  return !!name.trim() && name.length <= maxLength
}

function validateLogin(login) {
  // Проверка на заполненность и соответствие формату (например, только буквы и цифры)
  const emailRegex = /^[a-zA-Z0-9_]+$/
  return !!login.trim() && emailRegex.test(login)
}

function validatePassword(password) {
  // Проверка на минимальную длину и наличие разных типов символов
  const minLength = 8
