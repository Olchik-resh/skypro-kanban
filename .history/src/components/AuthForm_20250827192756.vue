...
function validatePassword(password) { return !!password.trim() } 
function clearError(fieldName) { errors.value[fieldName] = false } const isFormInvalid = computed(() => { const isNameValid = validateName(formData.value.name) const isLoginValid = validateLogin(formData.value.login) const isPasswordValid = validatePassword(formData.value.password) return !isNameValid || !isLoginValid || !isPasswordValid })
