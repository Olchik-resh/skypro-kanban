...
const validateForm = () => {
  ...
  if (!editedTask.value.date) {
    console.log('Дата:', editedTask.value.date)
console.log('Проверка:', dayjs(editedTask.value.date).isBefore(dayjs().startOf('day')))
console.log('dayjs().startOf("day"):', dayjs().startOf('day').toString())
    dateError.value = 'Укажите дату'
    valid = false
