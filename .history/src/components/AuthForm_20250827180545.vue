...
function validateName(name) {
  // Разрешить пустое значение на старте
  return !!name.trim() || name === ''
}

function validateLogin(login) {
  return !!login.trim() || login === ''
}
...
const isFormInvalid = computed(() => {
  ...
  updateErrors(isNameValid, isLoginValid, isPasswordValid)

  return !isNameValid || !isLoginValid || !isPasswordValid
})

function updateErrors(isNameValid, isLoginValid, isPasswordValid) {
  errors.value.name = !isNameValid
  errors.value.login = !isLoginValid
  errors.value.password = !isPasswordValid
}
...
const LOGIN_ERROR_MESSAGE =
  ...
  const ERROR_GENERAL_MESSAGE = 'Введенные вами данные не распознаны. Проверьте свой логин и пароль и повторите попытку входа.''
...
async function handleSubmit(event) {
  ...
  try {
    ...
    // Обработка ошибки, например, отображение сообщения об ошибке на экране
    error.value = ERROR_GENERAL_MESSAGE
  }
}
